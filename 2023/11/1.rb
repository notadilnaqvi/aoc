...
def get_dist_after_expansion(p1, p2, cols_with_no_galaxies, rows_with_no_galaxies)
  ...
  cols_with_no_galaxies.each do |row|
    if p1["x"] < p2["x"] and p1["x"] < row and row < p2["x"]
      # print(" row: #{row} is between #{p1["x"]} and #{p2["x"]}\n")
      expanded_cols_offset = expanded_cols_offset + 1
    elsif p2["x"] < p1["x"]  and p2["x"] < row and row < p1["x"]
      # print(" row: #{row} is between #{p2["x"]} and #{p1["x"]}\n")
      expanded_cols_offset = expanded_cols_offset + 1
    else
      # print(" row: #{row} is NOT between #{p2["x"]} and #{p1["x"]}\n")
    end
  end

  expanded_rows_offset = 0
  rows_with_no_galaxies.each do |col|
    if p1["y"] < p2["y"] and p1["y"] < col and col < p2["y"]
      # print(" col: #{col} is between #{p1["y"]} and #{p2["y"]}\n")
      expanded_rows_offset = expanded_rows_offset + 1
    elsif p2["y"] < p1["y"] and p2["y"] < col and col < p1["y"]
      # print(" col: #{col} is between #{p2["y"]} and #{p1["y"]}\n")
      expanded_rows_offset = expanded_rows_offset + 1
    else
      # print(" col: #{col} is NOT between #{p2["y"]} and #{p1["y"]}\n")
    end
  end
