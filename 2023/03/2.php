...
foreach ($matches[0] as $match) {
  ...
  foreach ($neighbours as $neighbour) {
    if ($neighbour['value'] === '*') {
      $part_members[$neighbour['x'].", ".$neighbour['y']][] = $matched_number;
      break;
    }
  }
}
...
$gear_ratios = [];
foreach ($part_members as $part_member) {
  if(count($part_member) == 2) {
    $gear_ratios[] = $part_member[0] * $part_member[1];
  }
}

$sum_of_gear_ratios = array_sum($gear_ratios);

print_r("[input.txt] $sum_of_gear_ratios\n");
...
function get_neighbours($index, $number_of_digits) {
  ...
  foreach ($neighbours_xys as $neighbour_xy) {
    ...
    if (($neighbour_x >= 0) and ($neighbour_y >= 0)  and ($neighbour_x < $col_count) and ($neighbour_y < $row_count)) {
      ...
      array_push($neighbours, array('value' => $neighbours_value, 'x' => $neighbour_x, 'y' => $neighbour_y));
    }
  }
  return $neighbours;
}
